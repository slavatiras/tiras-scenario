...
class AddNodeCommand(QUndoCommand):
    def __init__(self, scene, node_type_name, position, parent=None):
        ...
        # Проверяем, что мы в режиме редактирования макроса (дополнительная безопасность)
        main_window = next((v.parent() for v in self.scene.views() if hasattr(v, 'parent') and callable(v.parent)),
                           None)
        is_macro_mode = main_window.current_edit_mode == EDIT_MODE_MACRO if main_window else False
        ...
        self.main_window = main_window  # Уже получили его выше
        if self.main_window:
...
class AddNodeAndConnectCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        if not (start_socket and end_socket):
            log.error(
                f"AddNodeAndConnectCommand: Socket not found (start={start_socket}, end={end_socket}) for new node type {self.node_type_name}")
            self.setObsolete(True)
            if self.new_node.scene() == self.scene:
                self.scene.removeItem(self.new_node)
            return
...
class PasteCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        if not self.pasted_items_data and not self.pasted_connections_data:
            ...
            log.debug(
                f"PasteCommand redo: Scene check: has_input={scene_has_input}, has_output={scene_has_output}")  # Діагностика
            # --- [КІНЕЦЬ ЗМІНИ] ---
            ...
            try:
                ...
                for type_key, find_path in [('node', 'nodes/node'), ('comment', 'comments/comment'),
                                            ('frame', 'frames/frame')]:
                ...
                for el, item_type in elements_to_process:
                    item_data = None
                    if item_type == 'node':
                        item_data = BaseNode.data_from_xml(el)
                    elif item_type == 'comment':
                        item_data = CommentItem.data_from_xml(el)
                    elif item_type == 'frame':
                        item_data = FrameItem.data_from_xml(el)
                ...
                if connections_xml is not None and any(item['type'] == 'node' for item in self.pasted_items_data):
                    for conn_el in connections_xml:
                        ...
                        if conn_data['from_node'] in self.created_item_ids and conn_data[
                            'to_node'] in self.created_item_ids:
        ...
        for item_info in self.pasted_items_data:
            ...
            existing_item = next((item for item in self.scene.items() if hasattr(item, 'id') and item.id == new_id),
                                 None)
            if existing_item:
                if item_type == 'node':
                    created_nodes[new_id] = existing_item
                else:
                    created_others[new_id] = existing_item
            ...
            try:
                ...
                log.error(f"PasteCommand: Failed to create {item_type} from data: {item_data}. Error: {e}",
                          exc_info=True)
            ...
            if new_from_id and new_to_id:
                ...
                if from_node and to_node:
                    ...
                    if from_socket and to_socket:
                        ...
                        log.warning(f"PasteCommand redo: Sockets not found for connection: {conn_data}")
                else:
                    log.warning(
                        f"PasteCommand redo: Nodes not found for connection: {conn_data} (IDs: {new_from_id}, {new_to_id})")
            else:
                log.warning(f"PasteCommand redo: Old node IDs not mapped for connection: {conn_data}")
            ...
            if isinstance(start_node, BaseNode) and isinstance(end_node, BaseNode):
                ...
                if start_socket:
                    connection_to_remove = next(
                        (conn for conn in start_socket.connections if conn.end_socket == end_socket), None)
                if connection_to_remove:
                    if connection_to_remove.start_socket: connection_to_remove.start_socket.remove_connection(
                        connection_to_remove)
                    if connection_to_remove.end_socket: connection_to_remove.end_socket.remove_connection(
                        connection_to_remove)
                    if connection_to_remove.scene() == self.scene:
                        self.scene.removeItem(connection_to_remove)
...
class CreateMacroCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        self.removed_items_data = []
        items_to_remove = set()
        items_to_remove.update({item for item in selected_items if
                                isinstance(item, (BaseNode, CommentItem, FrameItem))})  # Include comments/frames
        ...
        for conn in connections_first:
            if conn.scene() == self.scene:
                try:  # Додаємо try-except
                    ...
                    log.error(
                        f"Error removing connection {conn.start_socket.parentItem().id if conn.start_socket else '?'}->{conn.end_socket.parentItem().id if conn.end_socket else '?'} in _remove_items_by_objects: {e}",
                        exc_info=True)
        ...
        for item in others:
            if item.scene() == self.scene:
                try:  # Додаємо try-except
                    ...
                    log.error(f"Error removing item {getattr(item, 'id', item)} in _remove_items_by_objects: {e}",
                              exc_info=True)
                    ...
                    if start_socket and end_socket:
                        ...
                        if not already_exists:
                            try:
                                ...
                                conn.update_path()
                                log.debug(
                                    f"Successfully restored connection {from_node_id} -> {to_node_id}")  # Діагностика
                            except Exception as e:
                                log.error(f"Error creating/adding connection in undo: {e}", exc_info=True)
                        else:
                            log.debug(f"Connection {from_node_id} -> {to_node_id} already exists.")  # Діагностика
                    else:
                        log.warning(f" Could not find sockets for restored connection: {conn_data}")
                else:
                    log.warning(f" Could not find nodes for restored connection: {conn_data}")
    ...
    def _create_macro_definition_and_analyze(self, selected_items):
        ...
        # --- [ИСПРАВЛЕНИЕ 3] Сортируем по Y-позиции ВНЕШНИХ узлов, чтобы избежать перекрещивания ---
        try:
            # x[2] - это conn_obj (объект Connection)
            ...
            log.debug("Sorted potential inputs/outputs by external node Y-position.")
        except Exception as e:
            log.error(
                f"Error sorting potential inputs/outputs by external node position: {e}. Falling back to internal sort.",
                exc_info=True)
            # Запасной вариант - сортировка по внутренним, как было
            potential_inputs.sort(key=lambda x: x[0])
        ...
        # --- [ИСПРАВЛЕНИЕ 1 и 2] Используем уникальные имена для входов/выходов ---
        input_name_base = "Вхід"
        output_name_base = "Вихід"
        input_idx = 1
        output_idx = 1
        # --- [КОНЕЦ ИСПРАВЛЕНИЯ] ---

        for i, (internal_y, original_conn_data, original_conn_obj) in enumerate(potential_inputs):
            # --- [ИСПРАВЛЕНИЕ 1 и 2] Генерируем уникальное имя
    ...
    def _create_and_add_macro_node(self, macro_data=None):
        ...
        node_positions = [QPointF(*item_info['data']['pos']) for item_info in self.removed_items_data if
                          item_info['type'] in ['node', 'comment', 'frame']]  # Include comments/frames
        if node_positions:
            try:
        ...
        for info in external_connections_info:
            ...
            try:
                if info['type'] == 'input':
                    ...
                    external_node = next((item for item in self.scene.items() if
                                          isinstance(item, BaseNode) and item.id == external_node_id), None)
                    if external_node:
                        ...
                        if external_socket and macro_socket:
                            ...
                            if not already_exists:
                                ...
                                log.debug(
                                    f"  Reconnected Input: {external_node_id}:{external_socket_name} -> Macro:{macro_socket_name}")
                        else:
                            log.warning(
                                f"  Input reconnect failed: Sockets not found (ext: {external_socket}, macro: {macro_socket}, name: {macro_socket_name})")
                    else:
                        log.warning(f"  Input reconnect failed: External node {external_node_id} not found.")
                    ...
                    if external_node:
                        ...
                        if external_socket and macro_socket:
                            ...
                            if not already_exists:
                                ...
                                log.debug(
                                    f"  Reconnected Output: Macro:{macro_socket_name} -> {external_node_id}:{external_socket_name}")
                        else:
                            log.warning(
                                f"  Output reconnect failed: Sockets not found (macro: {macro_socket}, ext: {external_socket}, name: {macro_socket_name})")
                    else:
                        log.warning(f"  Output reconnect failed: External node {external_node_id} not found.")

                if new_conn:
                    self.scene.addItem(new_conn)
                    new_connections_refs.append({
                        'start_ref': {'node_id': new_conn.start_socket.parentItem().id,
